...
from gql_preferences.utils.DBFeeder import get_demodata
...
async def Async_Session_Maker(DBModels):
    ...
    from gql_preferences.DBDefinitions import BaseModel
...
def LoadersContext(SQLite):
    from gql_preferences.utils.Dataloaders import createLoadersContext
...
def Context(AdminUser, SQLite, LoadersContext, Request):
    from gql_preferences.utils.gql_ug_proxy import get_ug_connection
...
def FastAPIClient(SQLite):
    ...
    import gql_preferences.DBDefinitions

    def ComposeCString():
        return "sqlite+aiosqlite:///:memory:"   
    gql_preferences.DBDefinitions.ComposeConnectionString = ComposeCString
