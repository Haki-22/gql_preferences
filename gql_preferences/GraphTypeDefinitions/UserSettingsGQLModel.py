...
class UserSettingsInsertGQLModel:
...
class UserSettingsUpdateGQLModel:
    id: Optional[UUID] = strawberry.field(description="primary key (UUID), could be client generated", default=None)
...
class UserSettingsDeleteGQLModel:
...
class UserSettingsResultGQLModel:
...
async def preference_user_settings_type_insert(self, info: strawberry.types.Info, user_settings: UserSettingsInsertGQLModel) -> UserSettingsResultGQLModel:
    ...
    row = next(rows, None)
    if row is not None:     
        return UserSettingsResultGQLModel(id=user_settings.id, msg="fail user already has settings of this type, (update?)")

    await loader.insert(user_settings)
    return UserSettingsResultGQLModel(id=user_settings.id, msg="OK, created")
...
async def preference_user_settings_type_update(self, info: strawberry.types.Info, user_settings: UserSettingsUpdateGQLModel) -> UserSettingsResultGQLModel:
    actingUser = getUser(info)
    loader = getLoaders(info).user_settings
    ...
    if row is None:
        return UserSettingsResultGQLModel(id=user_settings.id, msg="fail")
    
    return UserSettingsResultGQLModel(id=user_settings.id, msg="OK, updated")
...
async def preference_user_settings_delete(self, info: strawberry.types.Info, preference_settings: UserSettingsDeleteGQLModel) -> UserSettingsResultGQLModel:
    loader = getLoaders(info).user_settings

    rows = await loader.filter_by(id=preference_settings.id)
    row = next(rows, None)
    if row is None:     
        return UserSettingsResultGQLModel(id=preference_settings.id, msg="Fail bad ID")

    rows = await loader.filter_by(lastchange=preference_settings.lastchange)
    row = next(rows, None)
    if row is None:     
        return UserSettingsResultGQLModel(id=preference_settings.id, msg="Fail (bad lastchange?)")
    
    await loader.delete(preference_settings.id)
    
    return UserSettingsResultGQLModel(id=preference_settings.id, msg="OK, deleted")